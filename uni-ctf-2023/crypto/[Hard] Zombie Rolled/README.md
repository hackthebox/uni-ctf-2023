...
4. Mix the flag as follows:
   1. $s_1, s_2 = \operatorname{Sign}_K(flag)$
   2. Output $\text{mix} = (\operatorname{Encrypt}_K(s_1+s_2), \operatorname{Encrypt}_K(s_1-s_2))$
...
3. Compute $f=x/y=\operatorname{magic}(a,b,c)$ to be a part of the public key, where $\operatorname{magic}(x,y,z)$ is defined as $x/y+y/z+z/x$
4. Let $\text{nb}$ be the byte length of $y$.
5. Check if $\operatorname{magic}(p,q,r)=f$ holds. If not, go back to step 1.
...
1. Compute $h=\operatorname{SHA-256}(m)$
2. Let $a, b$ be $m, h$ respectively.
3. Generate a random number $c$ within $[0, 2^{nb})$.
4. Compute $r=\operatorname{magic}(a,b,c) \bmod{n}$
...
3. Compute $r=E_K(s_1)$
4. Compute $c=E_K(s_2)$
5. Check if $r=\operatorname{magic}(a,b,c) \bmod{n}$ holds. If so, return `True`, otherwise, return `False``.

It is easy to see `sign` and `verify` using RSA signing/verifying primitives $D_K$ and $E_K$ as a basic building block, then combine it with the $\operatorname{magic}$ function and some randomness to build a nondeterministic signature scheme.
...
Note that $\text{mix} = (\operatorname{Encrypt}_K(s_1+s_2), \operatorname{Encrypt}_K(s_1-s_2))$, so we can decrypt the `mix` list using the recovered private key to get:
...
with $a$ being the message and $b=\operatorname{SHA-256}(a)$.
...
So we have only one equation but with two unknowns $a,b$, and there is no easy way to express the relationship of $a=\operatorname{SHA-256}(b)$, so we couldn't solve this easily.
